...
@section('header')
    @switch($header)
        @case(str_contains($header, "index"))
            <h1 class="text-2xl font-bold text-center text-gray-900 dark:text-white">Lista de Juegos</h1>
            @break
        @case(str_contains($header, "show"))
            <h1 class="text-2xl font-bold text-center text-gray-900 dark:text-white">Detalles del Juegos</h1>
            @break
        @break
...
@section('content')
    @switch($header)
        @case(str_contains($header, "index"))
            <div class="container relative mx-auto px-4 mt-4">
                <div class="flex justify-center mt-4">
                    <a href="{{ route('game.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded underline">
                        Añadir Juego
                    </a>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-md md:max-w-xl lg:max-w-full py-3">
                    @foreach ($games as $game)
                        <div class=" flex flex-col justify-center bg-white dark:bg-gray-800 rounded-lg shadow p-4 mx-auto object-contain">
                            <img src="{{ $game->img_path  }}" alt="{{ $game->title }}" class="rounded-md mb-4" width="340px" height="200px">
        ...
        @case(str_contains($header, "show"))
            <div class="container mx-auto px-4 py-8 mt-4 ">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                    <h2 class="text-2xl text-white font-bold text-center">{{ $game->title }}</h2>
                    <div class="mt-4 flex gap-4 justify-center items-center max-w-sm md:max-w-md lg:max-w-lg mx-auto">
                        <img src="{{ $game->img_path }}" alt="{{ $game->title }}" class="object-contain rounded-md mb-4" width="50%" height="auto">
                        <div class="flex flex-col justify-center text-md md:text-base lg:text-xl">
                            <p class="text-gray-600 dark:text-gray-400 mb-3"><span class="text-white font-semibold">Fecha de estreno: </span>{{ date('d M Y', strtotime($game->release_date)) }}</p>
                            <p class="text-gray-600 dark:text-gray-400 mb-3"><span class="text-white font-semibold">Descripción: </span>{{ $game->description }}</p>
                            <p class="text-gray-600 dark:text-gray-400 mb-3"><span class="text-white font-semibold">Valoración media: </span>{{ $game->avg_score }}</p>
                            <div class="flex justify-center mt-4">
                                <a href="{{ route('game.index') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                                    Volver a la lista
                                </a>
                            </div>
                        </div>
        ...
        @case(str_contains($header, "create"))
            <div class="container mx-auto px-4 py-8 mt-4">
                <div class="flex-row justify-center">
                    <form action="{{ route('game.store') }}" method="POST" class="basis-md bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                        @csrf
                        <div class="mb-4">
        ...
        @case(str_contains($header, "edit"))
            <div class="container mx-auto px-4 py-8 mt-4">
                <div class="flex-row justify-center">
                    <form action="{{ route('game.update', $game->id) }}" method="POST" class="basis-md bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                            <label for="title" class="block text-md font-medium text-gray-700 dark:text-gray-300">Título</label>
                            <input type="text" name="title" id="title" value="{{ $game->title }}" class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        </div>
                        <div class="mb-4">
                            <label for="release_date" class="block text-md font-medium text-gray-700 dark:text-gray-300">Fecha de estreno</label>
                            <input type="date" name="release_date" id="release_date" value="{{ $game->release_date }}" class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        </div>
                        <div class="mb-4">
                            <label for="description" class="block text-md font-medium text-gray-700 dark:text-gray-300">Descripción</label>
                            <textarea name="description" id="description" rows="4" class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ $game->description }}</textarea>
                        </div>
                        <div class="mb-4">
                            <label for="img_path" class="block text-md font-medium text-gray-700 dark:text-gray-300">URL Imagen</label>
                            <input type="url" name="img_path" id="img_path" value="{{ $game->img_path }}" class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <div class="mb-4">
                            <label for="avg_score" class="block text-md font-medium text-gray-700 dark:text-gray-300">Valoración media</label>
                            <input type="number" name="avg_score" id="avg_score" step="0.1" min="0" max="10" value="{{ $game->avg_score }}" class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
